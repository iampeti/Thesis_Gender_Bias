...
ggplot(data, aes(x = genotype, y = expression)) +
  ...
  geom_hline(yintercept = beta_2, color = "red", size = 1.5, linetype = "dotted") +  # β₁ line
  labs(x = "factor", y = "expression") +
  theme_minimal(base_size = 14) +
  annotate("text", x = 0.5, y = beta_1, label = expression(beta[1]), color = "black", parse = TRUE, size = 5) +
  annotate("text", x = 0.7, y = beta_2, label = expression(beta[1]+beta[2]), color = "black", parse = TRUE, size = 5)
