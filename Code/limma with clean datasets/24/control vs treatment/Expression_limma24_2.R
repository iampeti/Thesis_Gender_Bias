...
design
colnames(design)=c("con_f_m","con_f_o","con_f_y","con_m_m","con_m_o","con_m_y","sch_f_m","sch_f_o","sch_f_y","sch_m_m","sch_m_o","sch_m_y")
design


contrasts=makeContrasts( (con_f_m+con_f_o+con_f_y+con_m_m+con_m_o+con_m_y) - (sch_f_m+sch_f_o+sch_f_y+sch_m_m+sch_m_o+sch_m_y)
...
res_coef1 = topTable(fit2, coef = 3, n= 10000)
...
if (l1 > 0) {
  write.table(geneNames[as.numeric(rownames(res_coef1)[1:l1])], "control vs schizo young", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "control vs schizo young", quote=F, row.names=F, col.names=F)
}


expr_levels = data.frame(geneNames[as.numeric(rownames(res_coef1)[1:l1])],res_coef1$adj.P.Val[1:l1])
write.csv(expr_levels, "expr_levels24_CvT_young.csv", row.names = FALSE)
