...
design
colnames(design)=c("fem_con_mid","fem_con_old","fem_con_you","fem_sch_mid","fem_sch_old","fem_sch_young","male_con_mid","male_con_old","male_con_you","male_sch_mid","male_sch_old","male_sch_young")
design


contrasts=makeContrasts( (fem_con_you-fem_sch_young)-(male_con_you-male_sch_young),
...
colnames(design2)=c("intercept","male","sch","mid","old","male_sch","male_mid","male_old","sch_mid","sch_old","male_sch_mid","male_sch_old")
design2
...
contrasts2 = makeContrasts(male_sch,   male + male_sch + male_sch_mid ,  male + male_sch + male_sch_old,  levels = design2)
contrasts2
...
write.csv(expr_levels, "expr_levels24_1.csv", row.names = FALSE)
...
if (l2_2 > 0) {
  write.table(geneNames[as.numeric(rownames(res2_2)[1:l2_2])], "with_intercept_female_vs_male_mid", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "with_intercept_female_vs_male_mid", quote=F, row.names=F, col.names=F)
}

expr_levels = data.frame(geneNames[as.numeric(rownames(res2_2)[1:l2_2])],res2_2$adj.P.Val[1:l2_2])
write.csv(expr_levels, "expr_levels24_2.csv", row.names = FALSE)
...
expr_levels = data.frame(geneNames[as.numeric(rownames(res2_3)[1:l2_3])],res2_3$adj.P.Val[1:l2_3])
write.csv(expr_levels, "expr_levels24_3.csv", row.names = FALSE)
