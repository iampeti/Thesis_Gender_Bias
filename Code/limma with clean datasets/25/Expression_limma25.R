...
colnames(design)=c("female_resistant_omental","female_resistant_subcutaneous","female_sensitive_omental","female_sensitive_subcutaneous","male_resistant_omental","male_resistant_subcutaneous")
design


contrasts=makeContrasts(female_resistant_omental-male_resistant_omental,female_resistant_subcutaneous-male_resistant_subcutaneous,levels=colnames(design))
contrasts
...
######## coef = 1 : female vs male in omental
######## coef = 2 : female vs male in subcutaneous
...
if (l1 > 0) {
  write.table(geneNames[as.numeric(rownames(res_coef1)[1:l1])], "female vs male omental", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "female vs male omental", quote=F, row.names=F, col.names=F)
}


expr_levels = data.frame(geneNames[as.numeric(rownames(res_coef1)[1:l1])],res_coef1$adj.P.Val[1:l1])
write.csv(expr_levels, "expr_levels25_1.csv", row.names = FALSE)
...
if (l2 > 0) {
  write.table(geneNames[as.numeric(rownames(res_coef2)[1:l2])], "female vs male subcutaneous",, quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "female vs male subcutaneous", quote=F, row.names=F, col.names=F)
}

expr_levels = data.frame(geneNames[as.numeric(rownames(res_coef2)[1:l2])],res_coef2$adj.P.Val[1:l2])
write.csv(expr_levels, "expr_levels25_2.csv", row.names = FALSE)
...
#######  with intercept, female resistant omental as reference level:
...
colnames(design2)=c("intercept","Males","Sensitive","Subcutaneous","Males_Sensitive","Males_Subcutaneous","Sensitive_Subcutaneous","Males_Sensitive_Subcutaneous")
design2

# intercept = female resistant omental
# Males =  effect of being male in resistant omental * coef=1
# Sensitive = effect of sensitive in omental in females
# Subcutaneous = effect of subcutaneous in resistant females
# Males_Sensitive = Male vs Female in Sensitive omental
# Males_Subcutaneous = Male vs Female in resistant in subcutaneous * coef=2
# Sensitive_Subcutaneous = Sensitive vs Resistant in females subcutaneous
# Male_Sensitive_Resistant = male vs female in resistant and sensitive


contrasts2 = makeContrasts(Males, Males+Males_Subcutaneous,  levels = design2)
contrasts2
