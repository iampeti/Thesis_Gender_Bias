...
group=paste(factors$disease_state,factors$gender,factors$tissue)

### Without intercept

design=model.matrix(~0+group)
design
colnames(design) = c("lean_f_sub","lean_f_vis","lean_m_sub","lean_m_vis","ob_f_sub","ob_f_vis","ob_m_sub","ob_m_vis")

design


contrasts=makeContrasts( (lean_f_sub+lean_f_vis+lean_m_sub+lean_m_vis)-(ob_f_sub+ob_f_vis+ob_m_sub+ob_m_vis), levels = colnames(design))
...
if (l1 > 0) {
  write.table(geneNames[as.numeric(rownames(res1)[1:l1])], "control vs treatment", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "control vs treatment", quote=F, row.names=F, col.names=F)
}

expr_levels = data.frame(geneNames[as.numeric(rownames(res1)[1:l1])],res1$adj.P.Val[1:l1])
write.csv(expr_levels, "expr_levels4_CvT.csv", row.names = FALSE)
