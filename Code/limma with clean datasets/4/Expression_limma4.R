...
colnames(design2)=c("intercept","Males","Obese","Visceral","Males_Obese","Males_Visceral","Obese_Visceral","Males_Obese_Visceral")
design2

# intercept = female lean subcutaneous
# Males =  difference between Males and Females in lean subcutaneous
# Obese = effect of obese in subcutaneous females
# Visceral =  effect of visceral in lean Females
# Males_Obese = captures how obese effect differs from males compared to females in subcutaneous
# Males_Visceral =  captures how visceral effect differs from males compared to females in lean
# Obese_Visceral = captures how visceral effect differs from lean to obese in females
# Males_Obese_Visceral = captures how obese effect differs from males to females in visceral


contrasts2 = makeContrasts(Males_Obese, Males_Obese_Visceral,  levels = design2)
contrasts2
...
######### coef = 1 : Males_Obese = captures how obese effect differs from males compared to females in subcutaneous
######### coef = 2 :Males_Obese_Visceral = captures how obese effect differs from males to females in visceral
...
if (l2 > 0) {
  write.table(geneNames[as.numeric(rownames(res2_1)[1:l2])], "with_intercept_female_vs_male_obese_subcutaneous", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "with_intercept_female_vs_male_obese_subcutaneous", quote=F, row.names=F, col.names=F)
...
if (l3 > 0) {
  write.table(geneNames[as.numeric(rownames(res2_2)[1:l3])], "with_intercept_female_vs_male_obese_visceral", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "with_intercept_female_vs_male_obese_visceral", quote=F, row.names=F, col.names=F)
  
}


expr_levels = data.frame(geneNames[as.numeric(rownames(res2_2)[1:l3])],res2_2$adj.P.Val[1:l3])
write.csv(expr_levels, "expr_levels4.csv", row.names = FALSE)
