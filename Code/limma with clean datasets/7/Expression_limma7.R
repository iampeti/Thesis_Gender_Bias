...
design
colnames(design) = c("f_o","f_y","m_o","m_y")
design


# female old vs Female young) vs (Male old VS Male young)
# how much the age  effect in males differs from the age effect in females
#

contrasts=makeContrasts((f_o-f_y)-(m_o-m_y), levels = colnames(design))
contrasts
...
if (l1 > 0) {
  write.table(geneNames[as.numeric(rownames(res1)[1:l1])], " (female old vs Female young) vs (Male old VS Male young)", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), " (female old vs Female young) vs (Male old VS Male young)", quote=F, row.names=F, col.names=F)
}



#######  with intercept, female old as reference level:

#factors$disease = relevel(factors$disease,"control")

design2 = model.matrix(~factors$gender*factors$age)
design2
colnames(design2)=c("intercept","Male","Young","Male_Young")
design2


# intercept = female control
# Male =  difference between males and females in old
# Young = effect of Young in females
# Male_Young = captures how young effect differs from males compared to females


contrasts2 = makeContrasts(Male_Young,  levels = design2)
contrasts2
...
######### captures how young effect differs from males compared to females
...
if (l2 > 0) {
  write.table(geneNames[as.numeric(rownames(res2)[1:l2])], "with_intercept_female_vs_male_young", quote=F, row.names=F, col.names=F)
} else {
