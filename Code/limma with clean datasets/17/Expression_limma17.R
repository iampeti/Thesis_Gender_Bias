...
design
colnames(design)=c("female_MA","female_OA","female_RA","female_SA","female_SLE","male_MA","male_OA","male_RA","male_SA")
design


contrasts=makeContrasts( female_MA-male_MA, female_OA-male_OA, female_RA-male_RA,female_SA-male_SA,
                         levels = colnames(design))
contrasts


# coef = 1: Microcrystalline
# coef = 2: Osteo
# coef = 3: Rheumatoid
# coef = 4: Seronegative
...
if (l1 > 0) {
  write.table(geneNames[as.numeric(rownames(res_coef1)[1:l1])], "female vs male Microcrystalline", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "female vs male Microcrystalline", quote=F, row.names=F, col.names=F)
}

expr_levels = data.frame(geneNames[as.numeric(rownames(res_coef1)[1:l1])],res_coef1$adj.P.Val[1:l1])
write.csv(expr_levels, "expr_levels17_1.csv", row.names = FALSE)
...
if (l2 > 0) {
  write.table(geneNames[as.numeric(rownames(res_coef2)[1:l2])],  "female vs male Osteo", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0),  "female vs male Osteo", quote=F, row.names=F, col.names=F)
}

expr_levels = data.frame(geneNames[as.numeric(rownames(res_coef2)[1:l2])],res_coef2$adj.P.Val[1:l2])
write.csv(expr_levels, "expr_levels17_2.csv", row.names = FALSE)
...
if (l3 > 0) {
  write.table(geneNames[as.numeric(rownames(res_coef3)[1:l3])],  "female vs male Rheumatoid", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0),  "female vs male Rheumatoid", quote=F, row.names=F, col.names=F)
}

expr_levels = data.frame(geneNames[as.numeric(rownames(res_coef3)[1:l3])],res_coef3$adj.P.Val[1:l3])
write.csv(expr_levels, "expr_levels17_3.csv", row.names = FALSE)
...
if (l4 > 0) {
  write.table(geneNames[as.numeric(rownames(res_coef4)[1:l4])], "female vs male Seronegative", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0),  "female vs male Seronegative", quote=F, row.names=F, col.names=F)
}

expr_levels = data.frame(geneNames[as.numeric(rownames(res_coef4)[1:l4])],res_coef4$adj.P.Val[1:l4])
write.csv(expr_levels, "expr_levels17_4.csv", row.names = FALSE)



#######  with intercept, female Microcrystalline as reference level:
...
colnames(design2)=c("intercept","Males","OA","RA","SA","SLE","Males_OA","Males_RA","Males_SA","Males_SLE")
design2

# intercept = female MA
# Males =  effect of Male in MA
# OA = effect of OA in females
# RA =  effect of RA females
# SA =  effect of SA in Females
# SLE = effect of SLE in females
# Males_OA = differences in males vs females in OA
# Males_RA = differences in males vs females in RA
# Males_SA = differences in males vs females in SA
# Males_Sle = difference in males vs females in SLE


contrasts2 = makeContrasts(Males, Males+Males_OA, Males+Males_RA, Males+Males_SA, levels = design2)
contrasts2
...
if (l2_1 > 0) {
  write.table(geneNames[as.numeric(rownames(res2_1)[1:l2_1])], "with_intercept_female_vs_male_MA", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "with_intercept_female_vs_male_MA", quote=F, row.names=F, col.names=F)
}
...
if (l2_2 > 0) {
  write.table(geneNames[as.numeric(rownames(res2_2)[1:l2_2])], "with_intercept_female_vs_male_OA", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "with_intercept_female_vs_male_OA", quote=F, row.names=F, col.names=F)
}
...
if (l2_3 > 0) {
  write.table(geneNames[as.numeric(rownames(res2_3)[1:l2_3])], "with_intercept_female_vs_male_RA", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "with_intercept_female_vs_male_RA", quote=F, row.names=F, col.names=F)
}
...
if (l2_4 > 0) {
  write.table(geneNames[as.numeric(rownames(res2_4)[1:l2_4])], "with_intercept_female_vs_male_SA", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "with_intercept_female_vs_male_SA", quote=F, row.names=F, col.names=F)
}
