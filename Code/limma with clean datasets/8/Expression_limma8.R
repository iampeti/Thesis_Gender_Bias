...
group = paste(factors$gender,factors$tissue, sep='_')
group

design=model.matrix(~0+group)
design
colnames(design)=c("female_left","female_right","male_left","male_right")
design


contrasts=makeContrasts( (female_left + female_right)*0.5 - (male_left + male_right)*0.5,
...
l1
res_coef1
geneNames[as.numeric(rownames(res_coef1)[1:l1])]
geneID_Refs[as.numeric(rownames(res_coef1)[1:l1])]


if (l1 > 0) {
  write.table(geneNames[as.numeric(rownames(res_coef1)[1:l1])], "female vs male", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "female vs male", quote=F, row.names=F, col.names=F)
}

expr_levels = data.frame(geneNames[as.numeric(rownames(res_coef1)[1:l1])],res_coef1$adj.P.Val[1:l1])
write.csv(expr_levels, "expr_levels8.csv", row.names = FALSE)



#######  with intercept, female leftl as reference level:

#factors$disease = relevel(factors$disease,"control")

design2 = model.matrix(~factors$gender*factors$tissue)
design2

colnames(design2)=c("intercept","Males","right","male_right")
design2

# intercept = female left
# Males =  effect of being male in left
# right = effect of right in females
# males_right = interaction effect of being male and right eye compared to female left eye

####
#(female left + female right)*05 = (intercept + intercept + right)*0.5
# (male left + male right)*05 = (intercept + males + intercept + males + right + Males_right)*0.5
# afairesh: Males+males_right*0.5

contrasts2 = makeContrasts(Males + (male_right)*0.5,  levels = design2)
contrasts2
