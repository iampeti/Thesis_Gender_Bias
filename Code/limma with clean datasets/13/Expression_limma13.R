...
contrasts=makeContrasts( groupfemale_young - groupmale_young , 
                         ...
                         groupfemale_very_old - groupmale_very_old ,
...
expr_levels1 = data.frame(geneNames[as.numeric(rownames(res_coef1)[1:l1])],res_coef1$adj.P.Val[1:l1])
write.csv(expr_levels1, "expr_levels13_1.csv", row.names = FALSE)
...
write.csv(expr_levels2, "expr_levels13_2.csv", row.names = FALSE)
...
write.csv(expr_levels3, "expr_levels13_3.csv", row.names = FALSE)
...
if (l4 > 0) {
  write.table(geneNames[as.numeric(rownames(res_coef4)[1:l4])], "female vs male very old", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0),  "female vs male very old", quote=F, row.names=F, col.names=F)
}

expr_levels4 = data.frame(geneNames[as.numeric(rownames(res_coef4)[1:l4])],res_coef4$adj.P.Val[1:l4])
write.csv(expr_levels4, "expr_levels13_4.csv", row.names = FALSE)
...
colnames(design2)=c("intercept","Males","ancient","middle","Old","Very_Old","Males_ancient","Males_middle","Males_old","Males_very_old")
design2
...
# very old =  effect of very old in Females
...
# Males_very_old = differences in males vs females in very old



contrasts2 = makeContrasts(Males, Males+Males_middle, Males+Males_old, Males+Males_very_old , levels = design2)
contrasts2
...
# coef = 4: Males + Males_very_old = differences in males vs females in very old
...
if (l2_4 > 0) {
  write.table(geneNames[as.numeric(rownames(res2_4)[1:l2_4])], "with_intercept_female_vs_male_very_old", quote=F, row.names=F, col.names=F)
} else {
  write.table(character(0), "with_intercept_female_vs_male_very_old", quote=F, row.names=F, col.names=F)
}
